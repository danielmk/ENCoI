...
# 'distal', 'intermediate', 'proximal', 'somatic', 'bombardment'
inp = 'intermediate'
...
for x in range(23,309):
    ...
    curr_dist = h.distance(h.d[x](0.5))
    dist_values.append(curr_dist)
    if curr_dist > 300.0:
        print("Epic Win")
        shape_plot1.color(4, sec=h.d[x])
...
if inp=='distal':
    ...
    inputs_e = np.random.choice(dist_idc_sorted[20:120], size=n_syne, replace=False)
    inputs_i = np.random.choice(dist_idc_sorted[20:120], size=n_syni, replace=False)
elif inp=='somatic':
    inputs_e = np.random.choice(dist_idc_sorted[:100], size=n_syne, replace=False)
    inputs_i = np.random.choice(dist_idc_sorted[:100], size=n_syni, replace=False)
