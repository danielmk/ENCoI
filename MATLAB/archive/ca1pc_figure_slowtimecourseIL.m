load('gm_somatic_dendrites_geslow.mat');
figure
FILTP = [20 0.0001 3 0.92]
[ge,gi,gl,re,VC,GT,Zt,cmm,ff,ff2,g1,g2,z1,z2] = find_gegi_optm(V(1:end-1),ac*1e-9,1/dt,[0 reversals(2) reversals(3)],[0.3 0.6], 'FILTP', FILTP);
...
load('sece_somatic_dendrites_geslow.mat')
ge_vclamp = g - mean(g(1:1000));
load('seci_somatic_dendrites_geslow.mat')
...
load('gm_somatic_dendrites_gislow.mat');
...
load('sece_somatic_dendrites_gislow.mat')
ge_vclamp = g - mean(g(1:1000));
load('seci_somatic_dendrites_gislow.mat')
gi_vclamp = g - mean(g(1:1000));
...
xlabel('time (s)')
ylabel('Voltage (V)')
title('Voltage With Dendrites Somatic Inputs ge=gi*1.5')
