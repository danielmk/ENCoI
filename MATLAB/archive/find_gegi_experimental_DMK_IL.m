function [ge,gi,gl,re,VC,GT,Zt,cmm,ff,ff2,g1,g2,z1,z2] = find_gegi(V,I,sf,reves,searchtime,varargin);
...
if NewExact3% solving based on absolute value:
    ...
    if boostcehere; %correcting for electrode capac.
        ...
        % estce = ce;
        kce = 1;
        gc1 = gt1-kce*1i*estce*2*pi*ff;
        gc2 = gt2-kce*1i*estce*2*pi*ff2;
        z1 = 1./gc1;
        z2 = 1./gc2;
    end
    ...
    %% backup  Rs_meas = (1/(2*(1i*p.c*w1 - 1i*p.c*w2)))*(1i*p.c*w1*z1a - 1i*p.c*w2*z1a + 1i*p.c*w1*z2a -...
      %  1i*p.c*w2*z2a + ((-1i*p.c*w1*z1a + 1i*p.c*w2*z1a - 1i*p.c*w1*z2a + 1i*p.c*w2*z2a).^2 -...
      %  4*(1i*p.c*w1 - 1i*p.c*w2)*(z1a - z2a + 1i*p.c*w1*z1a.*z2a - 1i*p.c*w2*z1a.*z2a)).^0.5);
    ...
    % we use mathematica :
    %%%
    %%Solve[Abs (re + 1/(g + I*w1*c)) == Abs (z1), {g}]
...
gii = (1.0*cdvdt+1*g0.*vll+gsyn2.*vee)./(vee-vii); %inhibition
