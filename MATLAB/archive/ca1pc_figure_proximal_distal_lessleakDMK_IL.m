...
figure
load('.\data\gm_distal_widthdendrites_lessleak_largersine.mat');

%reversals =[-0.065 0.00 -0.08];
FILTP = [20 0.0001 3 0.92]; % [DFF, dst, NC, STF1]
%reversals(3) = -0.09
[ge,gi,gl,re,VC,GT,Zt,cmm,ff,ff2,g1,g2,z1,z2] = find_gegi_experimental_DMK_IL(V(1:end-1),ac*1e-9,1/dt,[0 reversals(2) reversals(3)],[0.3 0.6], 'FILTP', FILTP);
cmm1_d = cmm;
gi1 = gi;
ge1 = ge;
gl1 = gl;
...
load('.\data\sece_distal_widthdendrites_lessleak_largersine.mat')
ge_vclamp = g - mean(g(1:1000));
load('.\data\seci_distal_widthdendrites_lessleak_largersine.mat')
...
xlabel('time (s)')
ylabel('Voltage (V)')
title('Voltage With Dendrites Distal')
